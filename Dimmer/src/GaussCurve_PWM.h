...
class GaussCurve {
  public:
    bool generateTableFromPoints(double points[][2], int numberpoints, int * lookuptable, int size_lookup);
    bool solve(double * mat, double * solution, int size);
    double plot(double * parameters, int size, double x);
    void generateLookupTable(double * parameters, int size, int * lookuptable, int size_lookup);
